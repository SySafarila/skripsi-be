    ...
    <div class="flex flex-col gap-4">
        ...
        <div class="flex flex-col">
            @forelse ($feedbacks as $key => $question)
                <div class="{{ $loop->index == 0 ? 'pb-2' : 'py-2' }} flex flex-col gap-2 border-b" id="question">
                    <div class="flex items-start justify-between gap-4">
                        {{-- <b>Kuesioner:</b> --}}
                        <span class="pt-1">{{ $key }}</span>
                        <button class="flex h-fit flex-shrink-0 cursor-pointer items-center gap-2 rounded-md px-2 py-1 hover:bg-gray-100"
